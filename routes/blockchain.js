...
router.get("/getTxOut/:txid/:n", config.blockchainRateLimit14, async (req, res, next) => {
  ...
  if (req.query.include_mempool && req.query.include_mempool === "true") include_mempool = true;

  requestConfig.data.id = "gettxout";
  requestConfig.data.method = "gettxout";
  requestConfig.data.params = [req.params.txid, parseInt(req.params.n), include_mempool];
